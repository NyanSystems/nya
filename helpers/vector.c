...
size_t vector_element_size(struct vector *vector) { return vector->esize; }

struct vector *vector_clone(struct vector *vector)
{
  void *new_data_address =
    calloc(vector->esize, vector->count + VECTOR_ELEMENT_INCREMENT);
...
int vector_current_index(struct vector *vector) { return vector->rindex; }

void vector_resize_for_index(struct vector *vector, int start_index,
                             int total_elements)
{
  ...
  vector->data = realloc(
    vector->data, ((start_index + total_elements + VECTOR_ELEMENT_INCREMENT) *
                   vector->esize));
  assert(vector->data);
...
void vector_peek_back(struct vector *vector) { vector->pindex--; }
...
void vector_set_flag(struct vector *vector, int flag) { vector->flags |= flag; }
...
void vector_save_purge(struct vector *vector) { vector_pop(vector->saves); }
...
const char *vector_string(struct vector *vec) { return vec->data; }
...
void vector_shift_right_in_bounds_no_increment(struct vector *vector, int index,
                                               int amount)
{
  vector_resize_for_index(vector, index, amount);
  int eindex = (index + amount);
  size_t bytes_to_move =
    vector_elements_until_end(vector, index) * vector->esize;
...
void vector_push_multiple_at(struct vector *vector, int dst_index, void *ptr,
                             int total)
{
                  ...
                  int dst_index)
{
  ...
  vector_push_multiple_at(vector_dst, dst_index, vector_at(vector_src, 0),
...
void vector_pop(struct vector *vector)
{
  ...
  // Popping from the vector will just decrement the index, no need to free
  // memory the next push will overwrite it.
  vector->rindex -= 1;
  vector->count -= 1;
...
void *vector_data_ptr(struct vector *vector) { return vector->data; }

bool vector_empty(struct vector *vector) { return vector_count(vector) == 0; }
...
int vector_count(struct vector *vector) { return vector->count; }